	...
	public static void main(String[] args) {
		...
		try {
			...
			//	DB의 위치(ip주소), 유저명, 비밀번호
			 conn
	     	=	DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "hr", "0000");//localhost 대신 ip주소 적어도됨
			
			//3. DB연결되면 쿼리문 작성
			//	쿼리문 실행하기 -> select의 경우 조회된 결과가 있음 : ResultSet
			// 60번 부서에 소속된 사원들의 정보 조회 - 사번, 성명, 부서코드, 
			// 업무코드, 전화번호, 이메일, 급여, 입사일자 
			 String sql = "select * from employees "
			 		+ "where department_id = " + department_id + " order by employee_id";
			 Statement st = conn.createStatement(); 
			 ResultSet rs = st.executeQuery(sql); // sqldeveloper에서 CTRL + ENTER
			 
			 //	조회된 결과를 눈으로 확인할 수 있게 출력문 작성
			 while(rs.next()) {//커서이동이 되면(가능하면)
